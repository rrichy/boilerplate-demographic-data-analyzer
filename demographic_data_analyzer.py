...
def calculate_demographic_data(print_data=True):
    ...
    df = pd.read_csv(CURR_DIR + '/adult.data.csv')
    ...
    rich_percentage = round(100 * num_min_workers.shape[0] / df[df['hours-per-week'] == min_work_hours].shape[0], 1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = (df[df['salary'] == '>50K']['native-country'].value_counts() / df['native-country'].value_counts()).idxmax()
    highest_earning_country_percentage = round(100 * df[(df['salary'] == '>50K') & (df['native-country'] == highest_earning_country)].shape[0] / df[df['native-country'] == highest_earning_country].shape[0], 1)
